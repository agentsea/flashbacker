...
export const sanitizeConversationBySections = tool({
  description: 'Section extractor. From sanitized text, locate H1–H3 headings and return bodies for requested labels (fuzzy or regex). Falls back to grep-like windows when headings don’t match. Example: sanitizeConversationBySections({ projectDir, sections: ["Project Memory","Current Working Plan"], useRegex: false }).',
  inputSchema: z.object({
    projectDir: z.string(),
    sections: z.array(z.string()).default(['Project Memory','Current Working Plan','Recent Conversation Log']),
    useRegex: z.boolean().default(false),
